<?php 
	...
	if(isset($_POST['submit_defaults'])){
		...
		$db_sql = sprintf("UPDATE `user_defaults` SET `Monday` = '%s',  `Tuesday` = '%s', `Wednesday`= '%s', `Thursday`= '%s', `Friday`= '%s', `Saturday`= '%s', `Sunday`= '%s' WHERE `user_defaults`.`UID`=$id ;", 
							$monday, $tuesday, $wednesday, $thursday, $friday, $saturday, $sunday);

		//TODO: Update projects too
		//, Mon_project = '%s',  Tues_project= '%s', Wed_project= '%s', Thurs_project= '%s', Fri_project= '%s', Sat_project= '%s', Sun_project= '%s', Saved_status= '%s', Submitted= '%s'
